...
#include "x86-64-netberg-aurora-615-sys.h"
...
ssize_t cpld_hw_ver_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    switch (attr->index)
    {
        case 23:
            ...
            {
                status = i2c_smbus_read_byte_data(Netberg_BMC_14_client, 0xc6);
            }
            else
            {
...
ssize_t usb_enable_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        if (i2c_smbus_read_byte_data(Netberg_CPLD_30_client, USB_EN_REG) & BIT_1_MASK)
        {
            sprintf(buf, "%s%d\n", buf, ENABLE);
        }
        else
