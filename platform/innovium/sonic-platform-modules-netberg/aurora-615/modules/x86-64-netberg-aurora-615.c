/* An hwmon driver for Netberg Aurora 615 Innovium i2c Module */
#pragma GCC diagnostic ignored "-Wformat-zero-length"
#include "x86-64-netberg-aurora-615.h"
...
static ssize_t psu_status_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    bmc_present = i2c_smbus_read_byte_data(NBA_615_i2c_client, BMC_PRESENT_OFFSET);
    if (bmc_present & mask)
    {
        status = i2c_smbus_read_byte_data(Netberg_BMC_client, 0xc0); //BMC 0x14 0xc0 
    }
    else
    {
        status = i2c_smbus_read_byte_data(Netberg_CPLD_5_client, 0xa0); //CPLD 0x35 0xa0 
    }
...
static ssize_t psu_module_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if (bmc_present & mask)
    {
        ...
        for(i = 0; i < 10; i ++)
        {
            u16_val = i2c_smbus_read_word_data(Netberg_BMC_client, psu_table[module_num][i]);
        ...
        /* vout mode */
        multiplier = 1000;
        u16_val = i2c_smbus_read_byte_data(Netberg_BMC_client, psu_table[module_num][10]);
        psu_status[10] = u16_val;   
        exponent = two_complement_to_int(u16_val & 0x1f, 5, 0x1f);
        /* vout */
        u16_val = i2c_smbus_read_word_data(Netberg_BMC_client, psu_table[module_num][2]);
...
static ssize_t dc_chip_switch_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        bmc_present = i2c_smbus_read_byte_data(NBA_615_i2c_client, BMC_PRESENT_OFFSET); //CPLD 0x30 0xa4
        if (bmc_present & mask)
        {
            for(i = 0; i < 12; i ++)
            {
                dc_status[i] = i2c_smbus_read_word_data(Netberg_BMC_client, dc_table[i]);
            }
...
static ssize_t usb_power_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    status = i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xa0);
...
static ssize_t usb_power_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    u16 i;
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
	struct Netberg_i2c_data *data = i2c_get_clientdata(NBA_615_i2c_client);
    ...
    if (attr->index == USB_POWER)
    {
        ...
        if (i == TURN_ON)
        {
            ...
            result = i2c_smbus_write_byte_data(NBA_615_i2c_client, 0xa0, value);
...
static ssize_t sys_led_ctrl_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    status = i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xa2); //to get register 0x30 0xa2
    ...
    status = i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xa3); //to get register 0x30 0xa3
...
static ssize_t sys_led_ctrl_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    led_value = i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xa2);
    debug_print((KERN_DEBUG "DEBUG : sys_led_ctrl_set led_value = %x\n",led_value));
    blk_value = i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xa3);
    ...
    if (attr->index != 0)
    {
        ...
        debug_print((KERN_DEBUG "DEBUG : sys_led_ctrl_set led_value = %x\n",led_value));
        debug_print((KERN_DEBUG "DEBUG : sys_led_ctrl_set blk_value = %x\n",blk_value));
        result  = i2c_smbus_write_byte_data(NBA_615_i2c_client, 0xa2, led_value);
        result |= i2c_smbus_write_byte_data(NBA_615_i2c_client, 0xa3, blk_value);
...
static ssize_t reset_mac_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    mutex_lock(&data->update_lock);
    debug_print((KERN_DEBUG "DEBUG : mutex_lock\n"));
    status  = i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xa1);
    ...
    if (attr->index == RESET_MAC)
    {
        ...
        if (i == 0)
        {
            ...
            result = i2c_smbus_write_byte_data(NBA_615_i2c_client, 0xa1, value);
...
static ssize_t themal_status_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    status = i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xc0);
...
static ssize_t themal_temp_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    int i;
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
	struct Netberg_i2c_data *Switch_2_data = i2c_get_clientdata(Netberg_Switch_2_client);
	struct Netberg_i2c_data *Sensor_data = i2c_get_clientdata(Netberg_Sensor_client);
    ...
    {
        channel_status = i2c_smbus_write_byte(Netberg_Switch_2_client, 0x02);
        if(channel_status < 0)
        {
            printk(KERN_ALERT "ERROR: themal_temp_get set channel 2 FAILED\n");
        }
        status = i2c_smbus_read_byte_data(Netberg_Sensor_client, 0X0);
        ...
        channel_status = i2c_smbus_write_byte(Netberg_Switch_2_client, 0x04);
        if(channel_status < 0)
        {
        ...
        channel_status = i2c_smbus_write_byte(Netberg_Switch_2_client, 0x08);
        if(channel_status < 0)
        {
        ...
        channel_status = i2c_smbus_write_byte(Netberg_Switch_2_client, 0x01);
        if(channel_status < 0)
        {
            printk(KERN_ALERT "ERROR: themal_temp_get set channel 1 FAILED\n");
        }
        status = i2c_smbus_read_byte_data(Netberg_Sensor_fan_client, 0X1);
    ...
    channel_status = i2c_smbus_write_byte(Netberg_Switch_2_client, 0x0);
    if(channel_status < 0)
    {
...
static ssize_t themal_mask_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    status = i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xc1);
...
static ssize_t themal_mask_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    result = i2c_smbus_write_byte_data(NBA_615_i2c_client, 0xc1, value);
...
static ssize_t int_status_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    res = 0x1;
    status = i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xd0);
...
static ssize_t sfp_status_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if (attr->index == SFP_PRESENT)
    {
        ...
        for (i = 1; i <= 6; i++)
        {
            ...
            {
                status = i2c_smbus_read_byte_data(Netberg_CPLD_2_client, present_reg[i]);
            }
            else
            {
                status = i2c_smbus_read_byte_data(Netberg_CPLD_3_client, present_reg[i]);
            }
    ...
    else if (attr->index == SFP_RX_LOSS)
    {
        ...
        for (i = 1; i <= 6; i++)
        {
            if(i <= 4) //for 1~32 port read 0x31 CPLD
            {
                status = i2c_smbus_read_byte_data(Netberg_CPLD_2_client, rx_loss_reg[i]);
            }
            else
            {
                status = i2c_smbus_read_byte_data(Netberg_CPLD_3_client, rx_loss_reg[i]);
            }
            debug_print((KERN_DEBUG "DEBUG : SFP_PRESENT status = %x\n",status));
            for (j = 1; j <= 8; j++)
            {
    ...
    else if (attr->index == SFP_TX_STAT)
    {
        ...
        for (i = 1; i <= 6; i++)
        {
            if(i <= 4) //for 1~32 port read 0x31 CPLD
            {
                status = i2c_smbus_read_byte_data(Netberg_CPLD_2_client, tx_ctrl_reg[i]);
            }
            else
            {
                status = i2c_smbus_read_byte_data(Netberg_CPLD_3_client, tx_ctrl_reg[i]);
            }
            debug_print((KERN_DEBUG "DEBUG : SFP_PRESENT status = %x\n",status));
            for (j = 1; j <= 8; j++)
            {
...
static ssize_t sfp_tx_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
    struct Netberg_i2c_data *CPLD_2_data = i2c_get_clientdata(Netberg_CPLD_2_client);
	struct Netberg_i2c_data *CPLD_3_data = i2c_get_clientdata(Netberg_CPLD_3_client);
    
    input = simple_strtol(buf, NULL, 10); //user input, 0 disable, 1 enable
    port_num = attr->index;
    if (port_num >= 1 && port_num <= 8)
    {
        target_client = Netberg_CPLD_2_client;
        reg = tx_ctrl_reg[1];
        offset = port_num;
    }
    ...
    {
        target_client = Netberg_CPLD_3_client;
        reg = tx_ctrl_reg[5];
        offset = port_num % 8;
    }
...
static ssize_t low_power_all_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    status = i2c_smbus_read_byte_data(Netberg_CPLD_3_client, 0x62); //CPLD 0x32 0x62
...
static ssize_t low_power_all_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    {
        i = simple_strtol(buf, NULL, 10);
        status = i2c_smbus_read_byte_data(Netberg_CPLD_3_client, 0x62);
        ...
        if (i == TURN_ON)
        {
            ...
            result = i2c_smbus_write_byte_data(Netberg_CPLD_3_client, 0x62, value);
...
static ssize_t low_power_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    if (i >= 1 && i <= 8)
    {
        ...
        if( j == TURN_ON)
        {
            ...
            result = i2c_smbus_write_byte_data(Netberg_CPLD_3_client, 0x62, status);
...
static ssize_t qsfp_reset_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    if (attr->index == QSFP_RESET)
    {
        ...
        {
            value  = 0;
            value  = i2c_smbus_read_byte_data(Netberg_CPLD_3_client, 0x72);
            debug_print((KERN_DEBUG "DEBUG : qsfp_reset_set value = %x\n",value));
            value ^= (1 << (i - 1));
            debug_print((KERN_DEBUG "DEBUG : qsfp_reset_set set value = %x\n",value));
            status = i2c_smbus_write_byte_data(Netberg_CPLD_3_client, 0x72, value);
            if (status < 0)
            {
...
static ssize_t qsfp_status_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        sprintf(buf, "");
        status = i2c_smbus_read_byte_data(Netberg_CPLD_3_client, 0x82);
        for (i = 1; i <= 8; i++)
        {
            if (status & res)
            {
    ...
    {
        sprintf(buf, "");
        status = i2c_smbus_read_byte_data(Netberg_CPLD_3_client, 0x92);
...
static ssize_t fan_status_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        if(i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xa4) == 0x1) //check BMC present
        {
            status = i2c_smbus_read_byte_data(Netberg_BMC_client, 0x80);
        }
        else
        {
            status = i2c_smbus_read_byte_data(Netberg_CPLD_4_client, 0x0);
        }
    ...
    else if (attr->index == FAN_PRESENT) 
    {
        if(i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xa4) == 0x1) //check BMC present
        {
            status = i2c_smbus_read_byte_data(Netberg_BMC_client, 0x81);
        }
        else
        {
            status = i2c_smbus_read_byte_data(Netberg_CPLD_4_client, 0x01);
        }
    ...
    else if (attr->index == FAN_POWER) 
    {
        if(i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xa4) == 0x1) //check BMC present
        {
            status = i2c_smbus_read_byte_data(Netberg_BMC_client, 0x82);
        }
        else
        {
            status = i2c_smbus_read_byte_data(Netberg_CPLD_4_client, 0x02);
        }
    ...
    else if (attr->index == FAN_SPEED_RPM)
    {
        ...
        if(i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xa4) == 0x1) //check BMC present
        {
            target_client = Netberg_BMC_client;
            res = i2c_smbus_read_byte_data(Netberg_BMC_client, 0x81); //check Fan present
        }
        else
        {
            target_client = Netberg_CPLD_4_client;
            res = i2c_smbus_read_byte_data(Netberg_CPLD_4_client, 0x01); //check Fan present
        }
...
static ssize_t fan_mode_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if (attr->index == FAN_MODE) 
    {
        ...
        status = i2c_smbus_read_byte_data(Netberg_Sensor_fan_client, 0x64);
        debug_print((KERN_DEBUG "DEBUG : FAN_MODE get status = %x\n",status));
        
        channel_status = i2c_smbus_write_byte(Netberg_Switch_2_client, 0x00);
        if(channel_status < 0)
        {
...
static ssize_t fan_mode_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    struct sensor_device_attribute *attr = to_sensor_dev_attr(da);
	struct Netberg_i2c_data *Switch_2_data = i2c_get_clientdata(Netberg_Switch_2_client);
	struct Netberg_i2c_data *Sensor_fan_data = i2c_get_clientdata(Netberg_Sensor_fan_client);
    mutex_lock(&Switch_2_data->update_lock);
    ...
    if (attr->index == FAN_MODE) 
    {
        ...
        status = i2c_smbus_write_byte_data(Netberg_Sensor_fan_client, 0x64, i);
...
static ssize_t fan_rpm_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if (attr->index == FAN_RPM) 
    {
        ...
        status = i2c_smbus_read_byte_data(Netberg_Sensor_fan_client, 0x60);
...
static ssize_t fan_rpm_set(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    if (attr->index == FAN_RPM) 
    {
        ...
        status = i2c_smbus_write_byte_data(Netberg_Sensor_fan_client, 0x60, i);
...
static ssize_t bmc_register_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    if(len == 1)
    {
        status = i2c_smbus_read_byte_data(Netberg_BMC_client, reg);
        debug_print((KERN_DEBUG "DEBUG : BMC byte status = 0x%x\n", status));
    }
    else if (len == 2)
    {
        status = i2c_smbus_read_word_data(Netberg_BMC_client, reg);
...
static ssize_t bmc_module_detect(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = i2c_smbus_read_byte_data(NBA_615_i2c_client, 0xa4);
        debug_print((KERN_DEBUG "DEBUG : BMC byte status = 0x%x\n", status));
    }
    if(status == 0x1)
    {
...
static ssize_t hw_version_get(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = i2c_smbus_read_byte_data(NBA_615_i2c_client, 0x20);
    }
...
{
    struct Netberg_i2c_data *data;
    struct Netberg_i2c_data *CPLD_2_data;
    struct Netberg_i2c_data *CPLD_3_data;
    struct Netberg_i2c_data *CPLD_4_data;
    struct Netberg_i2c_data *CPLD_5_data;
#ifdef I2C_SWITCH_WANTED
    struct Netberg_i2c_data *Switch_1_data;
    struct Netberg_i2c_data *Switch_2_data;
#endif
#ifdef THEMAL_WANTED
    struct Netberg_i2c_data *Sensor_data;
    struct Netberg_i2c_data *Sensor_fan_data;
#endif
#ifdef ASPEED_BMC_WANTED
    struct Netberg_i2c_data *Netberg_BMC_data;
#endif
    int status;
    ...
    data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!data)
    {
    ...
    CPLD_2_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!CPLD_2_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
    ...
    CPLD_3_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!CPLD_3_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
    ...
    CPLD_4_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!CPLD_4_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
    ...
    CPLD_5_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!CPLD_5_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
...
#ifdef I2C_SWITCH_WANTED
    Switch_1_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!Switch_1_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
    ...
    Switch_2_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!Switch_2_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
...
#endif
#ifdef THEMAL_WANTED
    Sensor_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!Sensor_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
    ...
    Sensor_fan_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!Sensor_fan_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
...
#endif
#ifdef ASPEED_BMC_WANTED
    Netberg_BMC_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!Netberg_BMC_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
    ...
    i2c_set_clientdata(Netberg_CPLD_2_client, CPLD_2_data);
    i2c_set_clientdata(Netberg_CPLD_3_client, CPLD_3_data);
    i2c_set_clientdata(Netberg_CPLD_4_client, CPLD_4_data);
    i2c_set_clientdata(Netberg_CPLD_5_client, CPLD_5_data);
#ifdef I2C_SWITCH_WANTED
    i2c_set_clientdata(Netberg_Switch_1_client, Switch_1_data);
    i2c_set_clientdata(Netberg_Switch_2_client, Switch_2_data);
#endif
#ifdef THEMAL_WANTED
    i2c_set_clientdata(Netberg_Sensor_client, Sensor_data);
    i2c_set_clientdata(Netberg_Sensor_fan_client, Sensor_fan_data);
#endif
...
#endif
#ifdef ASPEED_BMC_WANTED
    mutex_init(&Netberg_BMC_data->update_lock);
#endif
    data->valid = 0;
    mutex_init(&data->update_lock);
    dev_info(&client->dev, "chip found\n");
    /* Register sysfs hooks */
    status = sysfs_create_group(&client->dev.kobj, &NBA615_PSU_group);
    if (status)
    {
        goto exit_free;
    }
#ifdef USB_CTRL_WANTED
    status = sysfs_create_group(&client->dev.kobj, &NBA615_USB_group);
    if (status)
    {
        goto exit_free;
    }
#endif
    status = sysfs_create_group(&client->dev.kobj, &NBA615_LED_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &NBA615_Reset_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &NBA615_Sensor_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &NBA615_INT_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &NBA615_SFP_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &NBA615_QSFP_group);
    if (status)
    {
        goto exit_free;
    }
    status = sysfs_create_group(&client->dev.kobj, &NBA615_FAN_group);
    if (status)
    {
        goto exit_free;
    }
#ifdef ASPEED_BMC_WANTED
    status = sysfs_create_group(&client->dev.kobj, &NBA615_BMC_group);
    if (status)
    {
        goto exit_free;
    }
#endif
    status = sysfs_create_group(&client->dev.kobj, &NBA615_SYS_group);
    if (status)
    {
        goto exit_free;
    }
    ...
    return 0;
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &NBA615_PSU_group);
#ifdef USB_CTRL_WANTED
    sysfs_remove_group(&client->dev.kobj, &NBA615_USB_group);
#endif
    sysfs_remove_group(&client->dev.kobj, &NBA615_LED_group);
    sysfs_remove_group(&client->dev.kobj, &NBA615_Reset_group);
    sysfs_remove_group(&client->dev.kobj, &NBA615_Sensor_group);
    sysfs_remove_group(&client->dev.kobj, &NBA615_INT_group);
    sysfs_remove_group(&client->dev.kobj, &NBA615_SFP_group);
    sysfs_remove_group(&client->dev.kobj, &NBA615_QSFP_group);
    sysfs_remove_group(&client->dev.kobj, &NBA615_FAN_group);
#ifdef ASPEED_BMC_WANTED
    sysfs_remove_group(&client->dev.kobj, &NBA615_BMC_group);
#endif
    sysfs_remove_group(&client->dev.kobj, &NBA615_SYS_group);
...
static struct i2c_board_info NBA_615_i2c_info[] __initdata =
{
    {
        I2C_BOARD_INFO("NBA_615_i2c", 0x30),
        .platform_data = NULL,
    },
};

/*For QSFP Port 01 - 16*/
static struct i2c_board_info Netberg_CPLD_2_info[] __initdata =
{
    {
        I2C_BOARD_INFO("Netberg_CPLD_2", 0x31),
        .platform_data = NULL,
    },
};
/*For QSFP Port 17 - 32*/
static struct i2c_board_info Netberg_CPLD_3_info[] __initdata =
{
    {
        I2C_BOARD_INFO("Netberg_CPLD_3", 0x32),
        .platform_data = NULL,
    },
};
/*For Fan status*/
static struct i2c_board_info Netberg_CPLD_4_info[] __initdata =
{
    {
        I2C_BOARD_INFO("Netberg_CPLD_4", 0x23),
        .platform_data = NULL,
    },
};
/*For Power status*/
static struct i2c_board_info Netberg_CPLD_5_info[] __initdata =
{
    {
        I2C_BOARD_INFO("Netberg_CPLD_5", 0x35),
        .platform_data = NULL,
    },
};
#ifdef I2C_SWITCH_WANTED
/*0x73*/
static struct i2c_board_info Netberg_Switch_1_info[] __initdata =
{
    {
        I2C_BOARD_INFO("Netberg_Switch_1", 0x73),
        .platform_data = NULL,
    },
};

/*0x75*/
static struct i2c_board_info Netberg_Switch_2_info[] __initdata =
{
    {
        I2C_BOARD_INFO("Netberg_Switch_2", 0x75),
        .platform_data = NULL,
    },
};
#endif
#ifdef THEMAL_WANTED
/*0x4c*/
static struct i2c_board_info Netberg_Sensor_info[] __initdata =
{
    {
        I2C_BOARD_INFO("Netberg_Sensor", 0x4c),
        .platform_data = NULL,
    },
};

/*0x2e*/
static struct i2c_board_info Netberg_Sensor_fan_info[] __initdata =
{
    {
        I2C_BOARD_INFO("Netberg_Sensor_fan", 0x2e),
        .platform_data = NULL,
    },
};
#endif
#ifdef ASPEED_BMC_WANTED
static struct i2c_board_info Netberg_BMC_info[] __initdata =
{
    {
        I2C_BOARD_INFO("Netberg_BMC", 0x14),
        .platform_data = NULL,
    },
};
#endif
static int __init Netberg_i2c_init(void)
{
    ...
    debug_print((KERN_DEBUG "Netberg_i2c_init\n"));
    if (i2c_adap == NULL)
    {
        printk("ERROR: i2c_get_adapter FAILED!\n");
        return -1;
    }
    NBA_615_i2c_client = i2c_new_device(i2c_adap, &NBA_615_i2c_info[0]);
    Netberg_CPLD_2_client = i2c_new_device(i2c_adap, &Netberg_CPLD_2_info[0]);
    Netberg_CPLD_3_client = i2c_new_device(i2c_adap, &Netberg_CPLD_3_info[0]);
    Netberg_CPLD_4_client = i2c_new_device(i2c_adap, &Netberg_CPLD_4_info[0]);
    Netberg_CPLD_5_client = i2c_new_device(i2c_adap, &Netberg_CPLD_5_info[0]);
#ifdef I2C_SWITCH_WANTED
    Netberg_Switch_1_client = i2c_new_device(i2c_adap, &Netberg_Switch_1_info[0]);
    Netberg_Switch_2_client = i2c_new_device(i2c_adap, &Netberg_Switch_2_info[0]);
#endif
#ifdef THEMAL_WANTED
    Netberg_Sensor_client = i2c_new_device(i2c_adap, &Netberg_Sensor_info[0]);
    Netberg_Sensor_fan_client = i2c_new_device(i2c_adap, &Netberg_Sensor_fan_info[0]);
#endif
#ifdef ASPEED_BMC_WANTED
    Netberg_BMC_client = i2c_new_device(i2c_adap, &Netberg_BMC_info[0]);
#endif

    if (NBA_615_i2c_client == NULL || Netberg_CPLD_2_client == NULL || Netberg_CPLD_3_client == NULL 
    || Netberg_CPLD_4_client == NULL || Netberg_CPLD_5_client == NULL)
    {
        printk("ERROR: NBA_615_i2c_client FAILED!\n");
        return -1;
    }
#ifdef I2C_SWITCH_WANTED    
    if (Netberg_Switch_1_client == NULL || Netberg_Switch_2_client == NULL )
    {
        printk("ERROR: Netberg_Switch_client FAILED!\n");
        return -1;
    }
#endif
#ifdef THEMAL_WANTED
    if (Netberg_Sensor_client == NULL || Netberg_Sensor_fan_client == NULL )
    {
        printk("ERROR: Netberg_Sensor_client FAILED!\n");
        return -1;
    }
#endif
#ifdef ASPEED_BMC_WANTED
    if (Netberg_BMC_client == NULL )
    {
        printk("ERROR: Netberg_BMC_client FAILED!\n");
        return -1;
    }
#endif
    i2c_put_adapter(i2c_adap);
    ret = i2c_add_driver(&Netberg_i2c_driver);
    printk(KERN_ALERT "NBA615 i2c Driver Version: %s\n", DRIVER_VERSION);
    printk(KERN_ALERT "NBA615 i2c Driver INSTALL SUCCESS\n");
    return ret;
}

static void __exit Netberg_i2c_exit(void)
{
    i2c_unregister_device(NBA_615_i2c_client);
    i2c_unregister_device(Netberg_CPLD_2_client);
    i2c_unregister_device(Netberg_CPLD_3_client);
    i2c_unregister_device(Netberg_CPLD_4_client);
    i2c_unregister_device(Netberg_CPLD_5_client);
#ifdef I2C_SWITCH_WANTED
    i2c_unregister_device(Netberg_Switch_1_client);
    i2c_unregister_device(Netberg_Switch_2_client);
#endif
#ifdef THEMAL_WANTED
    i2c_unregister_device(Netberg_Sensor_client);
    i2c_unregister_device(Netberg_Sensor_fan_client);
#endif
#ifdef ASPEED_BMC_WANTED
    i2c_unregister_device(Netberg_BMC_client);
#endif
    i2c_del_driver(&Netberg_i2c_driver);
    printk(KERN_ALERT "NBA615 i2c Driver UNINSTALL SUCCESS\n");
}

MODULE_AUTHOR("Netberg Inc.");
MODULE_DESCRIPTION("Netberg NBA615 i2c Driver");
MODULE_LICENSE("GPL");
