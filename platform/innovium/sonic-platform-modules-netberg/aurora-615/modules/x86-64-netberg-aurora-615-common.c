...
#include "x86-64-netberg-aurora-615-common.h"
...
static int Netberg_i2c_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
    ...
    Netberg_CPLD_30_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!Netberg_CPLD_30_data)
    {
    ...
    Netberg_CPLD_31_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!Netberg_CPLD_31_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
        goto exit;
    ...
    Netberg_CPLD_32_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!Netberg_CPLD_32_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
        goto exit;
    ...
    Netberg_CPLD_23_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!Netberg_CPLD_23_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
        goto exit;
    ...
    Netberg_CPLD_35_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!Netberg_CPLD_35_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
        goto exit;
    ...
    Netberg_BMC_14_data = kzalloc(sizeof(struct Netberg_i2c_data), GFP_KERNEL);
    if (!Netberg_BMC_14_data)
    {
        printk(KERN_ALERT "kzalloc fail\n");
        status = -ENOMEM;
        goto exit;
    ...
    status = sysfs_create_group(&client->dev.kobj, &NBA615_THERMAL_group);
    if (status)
    {
        goto exit_free;
    }
    
    status = sysfs_create_group(&client->dev.kobj, &NBA615_POWER_group);
    if (status)
    {
        goto exit_free;
    }
...
exit_remove:
    ...
    sysfs_remove_group(&client->dev.kobj, &NBA615_THERMAL_group);
    sysfs_remove_group(&client->dev.kobj, &NBA615_POWER_group);
...
MODULE_DESCRIPTION("Netberg Aurora 615 i2c Driver");
MODULE_LICENSE("GPL");
