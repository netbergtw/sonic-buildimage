...
class Sfp(SfpBase):
    ...
    def __init__(self, index, eeprom_path_list, sfp_type, ext_sysfile_list=None):
        ...
        self.dom_supported = None
        self.dom_temp_supported = None
        self.dom_volt_supported = None
        self.dom_rx_power_supported = None
        self.dom_tx_power_supported = None
        ...
        if self.sfp_type == QSFP_TYPE:
            self.present_file = ext_sysfile_list[0]+ \
                'qsfp{}_present'.format(self.index- _qsfp_port_start)
            self.reset_file = ext_sysfile_list[0]+ \
                'qsfp{}_reset'.format(self.index- _qsfp_port_start)
            self.lp_file = ext_sysfile_list[0]+ \
                'qsfp{}_low_power'.format(self.index- _qsfp_port_start)
        else:
    ...
    def _get_sfp_type(self, sfp_type):
        ty = self._read_eeprom_bytes(SFP_ID_FIELDS['TYPE'][OFFSET],
                                     SFP_ID_FIELDS['TYPE'][WIDTH],
                                     INDEX_A0H)
        if ty is not None:
            ...
            if ty[0] in QSFP_TYPE_CODE_LIST:
                ...
                self.sfp_type = sfp_type
                logger.log_warning("Unreganized sfp type of module {} . \
                    unsupported, treated as specified type {}".format(self.index, sfp_type))
        else:
            self.sfp_type = sfp_type
    ...
    def _dom_capability_detect(self):
        ...
        if self.sfp_type == QSFP_TYPE:
            ...
            # QSFP capability byte parse, through this byte can know
            # whether it support tx_power or not.
            # TODO: in the future when decided to migrate to support SFF-8636 instead of SFF-8436,
            # need to add more code for determining the capability and version compliance
            # in SFF-8636 dom capability definitions evolving with the versions.
            qsfp_dom_capability_raw = self._read_eeprom_bytes((offset + XCVR_DOM_CAPABILITY_OFFSET),
                                                              XCVR_DOM_CAPABILITY_WIDTH, INDEX_A0H)
            if qsfp_dom_capability_raw is not None:
                qsfp_version_compliance_raw = \
                        self._read_eeprom_bytes(QSFP_VERSION_COMPLIANCE_OFFSET,
                ...
                self.calibration = 1
                qsfp_option_value_raw = self._read_eeprom_bytes(QSFP_OPTION_VALUE_OFFSET,
                                                                QSFP_OPTION_VALUE_WIDTH, INDEX_A0H)
                if qsfp_option_value_raw is not None:
                    ...
                    self.dom_tx_disable_supported = \
                        self.optional_capability['data']['TxDisable']['value'] == 'On'
            else:
                self.dom_supported = False
                self.dom_temp_supported = False
                self.dom_volt_supported = False
            ...
            if sfpi_obj is None:
                return None
            sfp_dom_capability_raw = self._read_eeprom_bytes(XCVR_DOM_CAPABILITY_OFFSET,
    ...
    def get_transceiver_info(self):
        ...
        sfp_interface_bulk_data = sfpi_obj.parse_sfp_info_bulk(sfp_interface_bulk_raw[start : end],
                                                               0)
        ...
        transceiver_info_dict['manufacturer'] = \
            sfp_vendor_name_data['data']['Vendor Name']['value']
        transceiver_info_dict['model'] = sfp_vendor_pn_data['data']['Vendor PN']['value']
        transceiver_info_dict['hardware_rev'] = sfp_vendor_rev_data['data']['Vendor Rev']['value']
        transceiver_info_dict['serial'] = sfp_vendor_sn_data['data']['Vendor SN']['value']
        transceiver_info_dict['vendor_oui'] = sfp_vendor_oui_data['data']['Vendor OUI']['value']
        transceiver_info_dict['vendor_date'] = \
            sfp_vendor_date_data['data']['VendorDataCode(YYYY-MM-DD Lot)']['value']
        transceiver_info_dict['connector'] = sfp_interface_bulk_data['data']['Connector']['value']
        transceiver_info_dict['encoding'] = \
            sfp_interface_bulk_data['data']['EncodingCodes']['value']
        transceiver_info_dict['ext_identifier'] = \
            sfp_interface_bulk_data['data']['Extended Identifier']['value']
        transceiver_info_dict['ext_rateselect_compliance'] = \
            sfp_interface_bulk_data['data']['RateIdentifier']['value']
        ...
        if self.sfp_type == QSFP_TYPE:
            ...
            transceiver_info_dict['nominal_bit_rate'] = \
                str(sfp_interface_bulk_data['data']['Nominal Bit Rate(100Mbs)']['value'])
        else:
    ...
    def get_transceiver_bulk_status(self):
        ...
        keys                       |Value   |
                                   |Format  |Information
        ---------------------------|--------|----------------------------
        rx_los                     |BOOLEAN |RX loss-of-signal status, True if has RX los,
                                            |False if not.
        tx_fault                   |BOOLEAN |TX fault status, True if has TX fault, False if not.
        reset_status               |BOOLEAN |reset status, True if SFP in reset, False if not.
        lp_mode                    |BOOLEAN |low power mode status, True in lp mode, False if not.
        tx_disable                 |BOOLEAN |TX disable status, True TX disabled, False if not.
        tx_disabled_channel        |HEX     |disabled TX channels in hex, bits 0 to 3 represent
        ...
        dom_temperature_raw = self._read_eeprom_bytes(field_offset + \
            id_field['TEMPERATURE'][OFFSET], id_field['TEMPERATURE'][WIDTH], path_idx)
        if dom_temperature_raw is not None:
            dom_temperature_data = sfpd_obj.parse_temperature(dom_temperature_raw, 0)
            transceiver_dom_info_dict['temperature'] = \
                dom_temperature_data['data']['Temperature']['value']

        dom_voltage_raw = self._read_eeprom_bytes(field_offset + id_field['VOLTAGE'][OFFSET],
                                                  id_field['VOLTAGE'][WIDTH], path_idx)
        if dom_voltage_raw is not None:
            dom_voltage_data = sfpd_obj.parse_voltage(dom_voltage_raw, 0)
            transceiver_dom_info_dict['voltage'] = dom_voltage_data['data']['Vcc']['value']

        dom_channel_monitor_raw = \
                self._read_eeprom_bytes(field_offset + id_field['CHANNEL_MON'][OFFSET],
                                        id_field['CHANNEL_MON'][WIDTH],
                                        path_idx)
        if dom_channel_monitor_raw is not None:
            if self.sfp_type == QSFP_TYPE:
                ...
                transceiver_dom_info_dict['tx2power'] = \
                    dom_channel_monitor_data['data']['TX2Power']['value']
                transceiver_dom_info_dict['tx3power'] = \
                    dom_channel_monitor_data['data']['TX3Power']['value']
                transceiver_dom_info_dict['tx4power'] = \
                    dom_channel_monitor_data['data']['TX4Power']['value']
                transceiver_dom_info_dict['rx1power'] = \
                ...
                transceiver_dom_info_dict['tx1bias'] = \
        ...
        for key in transceiver_dom_info_dict:
            transceiver_dom_info_dict[key] = \
                self._convert_string_to_num(transceiver_dom_info_dict[key])
    ...
    def get_transceiver_threshold_info(self):
        ...
        dom_module_threshold_raw = \
            self._read_eeprom_bytes(field_offset + id_field['MODULE_THRESHOLD'][OFFSET],
                                    id_field['MODULE_THRESHOLD'][WIDTH],
                                    path_idx)
        ...
        if self.sfp_type == QSFP_TYPE:
            dom_module_threshold_data = \
                sfpd_obj.parse_module_threshold_values(dom_module_threshold_raw, 0)

            dom_channel_threshold_raw = \
                    self._read_eeprom_bytes(field_offset + id_field['CHANNEL_THRESHOLD'][OFFSET],
                                            id_field['CHANNEL_THRESHOLD'][WIDTH],
                                            path_idx)

            if dom_channel_threshold_raw is None:
                return transceiver_dom_threshold_info_dict

            dom_channel_threshold_data = \
                sfpd_obj.parse_channel_threshold_values(dom_channel_threshold_raw, 0)
            ...
            dom_module_threshold_data = \
                sfpd_obj.parse_alarm_warning_threshold(dom_module_threshold_raw, 0)
            transceiver_dom_threshold_info_dict['rxpowerhighalarm'] = \
        ...
        transceiver_dom_threshold_info_dict['vcchighalarm'] = \
            dom_module_threshold_data['data']['VccHighAlarm']['value']
        transceiver_dom_threshold_info_dict['vcchighwarning'] = \
            dom_module_threshold_data['data']['VccHighWarning']['value']
        transceiver_dom_threshold_info_dict['vcclowalarm'] = \
    ...
    def get_power_override(self):
        ...
        if self.sfp_type == QSFP_TYPE:
            ...
            dom_control_raw = self._read_eeprom_bytes(id_field['CONTROL'][OFFSET],
                                                      id_field['CONTROL'][WIDTH],
                                                      INDEX_A0H)
    ...
    def get_temperature(self):
        ...
        dom_temperature_raw = \
            self._read_eeprom_bytes(field_offset + id_field['TEMPERATURE'][OFFSET],
                                    id_field['TEMPERATURE'][WIDTH],
                                    path_idx)
        if dom_temperature_raw is not None:
            dom_temperature_data = sfpd_obj.parse_temperature(dom_temperature_raw, 0)
    ...
    def get_voltage(self):
        ...
        dom_voltage_raw = \
            self._read_eeprom_bytes(field_offset + id_field['VOLTAGE'][OFFSET],
                                    id_field['VOLTAGE'][WIDTH],
                                    path_idx)
        if dom_voltage_raw is not None:
            dom_voltage_data = sfpd_obj.parse_voltage(dom_voltage_raw, 0)
