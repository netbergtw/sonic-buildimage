...
class Chassis(ChassisBase):
    ...
    def __init__(self):
        ...
        if self.platdev.get_fan_support():
            ...
            for index, fan_name in enumerate(fanlist):
        ...
        for index in range(0, len(thermal_info_list)):
            if len(self.platdev.get_thermal_dev_tempidx_by_idx(index)) > 1:
                for idx in self.platdev.get_thermal_dev_tempidx_by_idx(index):
                    thermal = Thermal(idx,
                                      self.platdev.get_thermal_dev_name_by_idx(index)+"-{}".format(idx),
    ...
    def _get_sku_name(self):
        pipe = subprocess.Popen(GET_HWSKU_CMD, shell=True, stdout=subprocess.PIPE)
        out, err = pipe.communicate()
        return out.decode().rstrip('\n')


    def _get_platform_name(self):
        pipe = subprocess.Popen(GET_PLATFORM_CMD, shell=True, stdout=subprocess.PIPE)
        out, err = pipe.communicate()
        return out.decode().rstrip('\n')
    ...
    def sfp_debugger(self):
        ...
        for sfp_n in range(0, len(self._sfp_list)):
            print("======SFP{}==TYPE {}====".format(sfp_n, self._sfp_list[sfp_n].sfp_type))
        ...
        print("get_lpmode:")
        for sfp_n in range(0, len(self._sfp_list)):
            print("\tsfp{}: {}".format(sfp_n, self._sfp_list[sfp_n].get_lpmode()))
            # set_lpmode

        print("get_power_override:")
        for sfp_n in range(0, len(self._sfp_list)):
            print("\tsfp{}: {}".format(sfp_n, self._sfp_list[sfp_n].get_power_override()))

        print("get_temperature:")
        for sfp_n in range(0, len(self._sfp_list)):
            print("\tsfp{}: {}".format(sfp_n, self._sfp_list[sfp_n].get_temperature()))

        print("get_voltage")
        for sfp_n in range(0, len(self._sfp_list)):
            print("\tsfp{}: {}".format(sfp_n, self._sfp_list[sfp_n].get_voltage()))

        print("get_tx_bias")
        for sfp_n in range(0, len(self._sfp_list)):
            print("\tsfp{}: {}".format(sfp_n, self._sfp_list[sfp_n].get_tx_bias()))

        print("get_rx_power")
        for sfp_n in range(0, len(self._sfp_list)):
            print("\tsfp{}: {}".format(sfp_n, self._sfp_list[sfp_n].get_rx_power()))

        print("get_tx_power")
        for sfp_n in range(0, len(self._sfp_list)):
            print("\tsfp{}: {}".format(sfp_n, self._sfp_list[sfp_n].get_tx_power()))
