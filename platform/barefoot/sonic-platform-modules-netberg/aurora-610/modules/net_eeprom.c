...
static ssize_t net_eeprom_read(struct file *filp, struct kobject *kobj,
			   ...
			   char *buf, loff_t off, size_t count)
{
	...
	for (slice = off >> SLICE_BITS; slice <= (off + count - 1) >> SLICE_BITS; slice++) {
		net_eeprom_update_client(client, slice);
	}
...
static struct bin_attribute net_eeprom_attr = {
	.attr = {
		.name = "eeprom",
		.mode = S_IRUGO,
	},
...
static int net_eeprom_probe(struct i2c_client *client,
			const struct i2c_device_id *id)
{
	...
	err = sysfs_create_bin_file(&client->dev.kobj, &net_eeprom_attr);
	if (err) {
		goto exit_kfree;
	}
...
static struct i2c_driver net_eeprom_driver = {
	.driver = {
		.name	= "net_eeprom",
	},
...
module_i2c_driver(net_eeprom_driver);

MODULE_AUTHOR("Netberg");
MODULE_DESCRIPTION("Mother Board EEPROM driver");
MODULE_LICENSE("GPL");
